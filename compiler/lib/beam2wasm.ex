...
# + emit path; the WAT runtime library is in Codegen.Runtime, shared leaf helpers in Codegen.Common.
...
defmodule Beam2Wasm do
  ...
  def run(beam_paths) do
    ...
    atom_names? = Enum.any?(user, fn {_m, {:function, _, _, _, is}} ->
      Enum.any?(is, fn op ->
        match?({_, _, {:extfunc, :erlang, f, _}} when f in [:atom_to_binary, :binary_to_existing_atom, :list_to_existing_atom], op) or
          match?({_, _, {:extfunc, :erlang, f, _}, _} when f in [:atom_to_binary, :binary_to_existing_atom, :list_to_existing_atom], op)
      end)
    ...
    regex_run3? = Enum.any?(user, fn {_m, {:function, _, _, _, is}} ->
      Enum.any?(is, fn op ->
    ...
    uninorm? = Enum.any?(user, fn {_m, {:function, _, _, _, is}} ->
      Enum.any?(is, fn op ->
        match?({_, _, {:extfunc, :unicode, f, 1}} when f in [:characters_to_nfc_binary, :characters_to_nfd_binary, :characters_to_nfkc_binary, :characters_to_nfkd_binary], op) or
          match?({_, _, {:extfunc, :unicode, f, 1}, _} when f in [:characters_to_nfc_binary, :characters_to_nfd_binary, :characters_to_nfkc_binary, :characters_to_nfkd_binary], op)
      end)
    end)
    ...
    fltfmt? = flt and Enum.any?(user, fn {_m, {:function, _, _, _, is}} ->
      Enum.any?(is, fn op ->
    ...
    extra_defined =
      (if atom_names?,
         do: [{:erlang, :atom_to_binary, 1}, {:erlang, :atom_to_binary, 2},
              {:erlang, :binary_to_existing_atom, 1}, {:erlang, :binary_to_existing_atom, 2},
  ...
  def reg_writes({:get_map_elements, _, _, {:list, _}} = _op), do: []   # dests are inside the list; treat as :other below
  ...
  def float_op?(f) when is_float(f), do: true   # a bare float literal operand (e.g. {:float, 0.25}) → float mode
  ...
  def make_fun_refs({:make_fun3, {m, fun, arity}, _i, _h, _d, {:list, free}}), do: [{m, fun, arity, length(free)}]
  ...
  def literal_funs_in(m) when is_map(m), do: Map.to_list(m) |> Enum.flat_map(fn {k, v} -> literal_funs_in(k) ++ literal_funs_in(v) end)
  ...
  def capture_wrap_body({:erlang, :byte_size, 1}), do: "(ref.i31 (array.len (struct.get $binary 0 (ref.cast (ref $binary) #{x(0)}))))"
  def capture_wrap_body({:erlang, :bit_size, 1}), do: "(ref.i31 (i32.shl (array.len (struct.get $binary 0 (ref.cast (ref $binary) #{x(0)}))) (i32.const 3)))"
  def capture_wrap_body({:erlang, :map_size, 1}), do: "(ref.i31 (call $map_size #{x(0)}))"
  def capture_wrap_body({:erlang, :tuple_size, 1}), do: "(ref.i31 (array.len (ref.cast (ref $tuple) #{x(0)})))"
  ...
  def term_atoms(m) when is_map(m), do: Map.to_list(m) |> Enum.flat_map(fn {k, v} -> term_atoms(k) ++ term_atoms(v) end)
