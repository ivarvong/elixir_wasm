# Codegen.Emit — the per-function BEAM->WAT emit path: compile_fun/2 (the opcode case), value/operand
# renderers, arith/comparison/bitstring/map/const helpers, and instruction pre-processing. Extracted from
# beam2wasm.ex (defp -> def); imports Codegen.Common. Independent of the WAT runtime library.
defmodule Codegen.Emit do
      ...
      case op do
        ...
        {:get_map_elements, {:f, fail}, src, {:list, pairs}} ->
          ...
          lines =
            ["(local.set $tmp #{val.(src)})"] ++
              (pairs |> Enum.chunk_every(2) |> Enum.flat_map(fn [key, dst] ->
        ...
        {:bs_match, {:f, fail}, ctx, {:commands, cmds}} ->
          ...
          lines = Enum.flat_map(cmds, fn
            ...
            {:float, _live, _flags, size, unit, dst} when size * unit == 64 ->
              ["(local.set $bsrc (struct.get $mctx 0 (local.get $mc)))",
               set.(dst, "(struct.new $float (f64.reinterpret_i64 (call $bits_read64 (local.get $bsrc) (struct.get $mctx 1 (local.get $mc)) (i32.const 64))))"),
        ...
        {:bif, :element, {:f, f}, [idx, src], d} when f != 0 ->
          {["(if (i32.eqz (ref.test (ref $tuple) #{val.(src)})) (then #{jump.(f)}))",
            ...
            set.(d, "(array.get $tuple (ref.cast (ref $tuple) #{val.(src)}) (i32.sub #{i32v.(idx)} (i32.const 1)))")], false}
  ...
  def operand(nil), do: "(ref.null none)"                       # bare nil operand = the empty list []
  def operand({:atom, nil}), do: "(global.get $atom_nil)"       # the atom nil (Elixir's nil) — distinct
  ...
  def seg_len([{:atom, t}, _fl, _u, _n, src, {:atom, :all}], val) when t in [:binary, :append, :private_append],
  ...
  def seg_len([{:atom, :integer}, _fl, u, _n, _src, {:integer, sz}], _val), do: "(i32.const #{div(sz * u, 8)})"
  def seg_len([{:atom, :utf8}, _fl, _u, _n, src, {:atom, :undefined}], _val), do: "(call $utf8_enc_len #{i32val(src)})"
  ...
  def key_term(nil), do: {:ok, []}              # bare nil operand = the empty list
  ...
  def materialize(nil), do: "(global.get $atom_nil)"   # the atom nil (distinct from [] below)
