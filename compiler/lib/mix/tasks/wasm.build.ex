defmodule Mix.Tasks.Wasm.Build do
  ...
  @stdlib [
    ...
    Integer, Float, Tuple, Range, Stream, Enumerable, Collectable, Inspect,
    Inspect.Algebra, Access, ArgumentError, RuntimeError, KeyError,
    :lists, :maps, :sets, :ordsets, :gb_sets, :proplists, :orddict,
    :erl_scan, :erl_anno, :string, :io_lib, :io_lib_format, :io_lib_pretty,
    Enumerable.List, Enumerable.Map, Enumerable.Range, Enumerable.MapSet, Enumerable.Function,
    Enumerable.Stream, Collectable.List, Collectable.Map, Collectable.MapSet,
    Collectable.BitString, String.Chars.Integer, String.Chars.Float, String.Chars.List,
    String.Chars.BitString, String.Chars.Atom, List.Chars.BitString, List.Chars.Integer,
    List.Chars.List, List.Chars.Atom
  ]
  ...
  def run(args) do
    {opts, _} =
      OptionParser.parse!(args,
        strict: [module: :string, export: :keep, out: :string, worker: :boolean,
                 strict: :boolean, stdlib: :boolean]
      )
    ...
    if compile_stubs > 0 or externals != [] do
      ...
      if length(externals) > 15, do: Mix.shell().info("        … #{length(externals) - 15} more (`;; stub: external` markers in the .wat)")
      Mix.shell().info("""
      ...
      """)
      if opts[:strict] do
        Mix.raise("--strict: #{compile_stubs} unsupported constructs + #{length(externals)} missing externals (0/0 = provably supported)")
      end
    end
  ...
  defp collect_beams(module, stdlib?) do
    ...
    primary = Enum.find(app_beams, &(Path.basename(&1) == "#{module}.beam")) ||
  ...
  defp compile(beams, exports_spec) do
    # Beam2Wasm reads its modes from the environment; STUB=1 so unsupported functions become
    # COUNTED traps we can report (and fail on) instead of aborting mid-module.
    System.put_env("EXPORTS", exports_spec)
    System.put_env("STUB", "1")
    System.put_env("BIGNUM", "1")
    # run in a fresh process: the compiler keys its build state off the process dictionary.
    # :stubs (its honest count of unsupported constructs) lives in that dictionary too.
    Task.async(fn -> {Beam2Wasm.run(beams), Process.get(:stubs, 0)} end) |> Task.await(:infinity)
  end
    ...
    manifest =
      ...
      |> Enum.map(fn spec ->
        ...
        unless unsupported == [] do
          Mix.raise("--worker can wire int/bin/float signatures over HTTP; #{name} uses #{inspect(unsupported)}")
        end
