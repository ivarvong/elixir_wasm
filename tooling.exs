...
defmodule Tooling do
  ...
  # ── stdlib closure for feeding ───────────────────────────────────────────────

  # Host-shimmed modules: the compiler defines these itself, so feeding their .beam
  # double-defines functions ("repeated function name"). Everything else is fair game —
  # the compiler's function-level DCE prunes whatever the export seeds don't reach, so a
  # broad feed costs nothing in output size (proven: mergesort + 12 unrelated modules ->
  # byte-identical WAT). Only the shim collision and disasm time are real costs.
  @shimmed [Regex, :re, :binary, :unicode, :math, :string, :erlang, :os, :file, :io,
            :rand, :crypto, :ets, :persistent_term,
            # String case-mapping/normalization is host-shimmed; feeding the generated data
            # modules double-defines downcase_3/upcase_3 etc.
            String.Unicode, String.Casing, String.Normalizer]
  @meta_prefixes ~w(IEx Mix Code Macro Kernel.CLI Kernel.ParallelCompiler Kernel.LexicalTracker
                    Module.ParallelChecker Module.Types)

  @doc """
  A broad, feedable beam list: every Elixir stdlib module plus the common pure Erlang
  stdlib, MINUS the host-shimmed modules and compile-time/meta modules. Feed this instead
  of hand-curating per program — DCE keeps only what the exports reach. Pass `extra_deny`
  (a list of module atoms) to drop more.
  """
  def stdlib_closure(extra_deny \\ []) do
    deny = MapSet.new(@shimmed ++ extra_deny)
    erl = [:lists, :maps, :sets, :ordsets, :gb_sets, :filename, :proplists, :orddict, :array, :queue, :calendar]

    (Application.spec(:elixir, :modules) ++ erl)
    |> Enum.reject(fn m -> MapSet.member?(deny, m) or meta_module?(m) end)
    |> Enum.map(fn m -> Code.ensure_loaded(m); :code.which(m) end)
    |> Enum.filter(&(is_list(&1) and List.to_string(&1) |> String.ends_with?(".beam")))
    |> Enum.map(&List.to_string/1)
    |> Enum.uniq()
  end

  defp meta_module?(m) do
    s = inspect(m)
    Enum.any?(@meta_prefixes, &(s == &1 or String.starts_with?(s, &1 <> ".")))
  end
