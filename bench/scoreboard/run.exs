...
defmodule Scoreboard do
  ...
  @roster [Enum, List, Map, Keyword, Tuple, Integer, String, Range, MapSet, Float,
           Atom, Bitwise, Access, Function, Date, Time, NaiveDateTime]

  # Frontier modules probed by `run.exs all` (coverage map, NOT the pinned 100% gate). As each
  # reaches 100%, it graduates into @roster above and the verify.exs floor rises.
  @explore [Base, Path, Version, Regex, DateTime, Calendar, Duration, Date.Range]
  ...
  @pool1 ["[3,1,2,2]", ~s("hello world"), "%{a: 1, b: 2}", "{1, 2, 3}", "5", "-7", "3.5", "1..5",
          "[a: 1, b: 2]", ":an_atom", ~s(""), "[]", "mapset1", "date1", "time1", "ndt1", "dt1"]
  ...
  @slot ["[3,1,2,2]", ~s("hello world"), ~s("o"), "%{a: 1, b: 2}", "{1, 2, 3}", "5", "2", ":a",
         ...
         "date1", "time1", "ndt1", "dt1"]
  @aliases %{"fun1" => "&(&1 * 2)", "fun2" => "fn a, b -> a + b end", "mapset1" => "MapSet.new([1, 2, 3])",
             "date1" => "~D[2024-01-15]", "time1" => "~T[10:30:00]",
             "ndt1" => "~N[2024-01-15 10:30:00]", "dt1" => "~U[2024-01-15 10:30:00Z]"}
  # genuinely nondeterministic functions: equal output across runs/impls is not even defined.
  # PRNG, plus clock-readers (utc_now/utc_today/now read the wall clock — "today" differs per run).
  @nondet [random: 1, shuffle: 1, take_random: 2, unique_integer: 0,
           utc_today: 0, utc_now: 0, utc_now: 1, now: 1, local_now: 0]
  ...
  def main(args) do
    ...
    roster =
      cond do
        filter in ["all", "full"] -> @roster ++ @explore
        filter -> Enum.filter(@roster ++ @explore, &(inspect(&1) == filter))
        true -> @roster
      end
  ...
  defp score_module(mod) do
    ...
    # A module can fail to compile to Wasm entirely (e.g. a codegen bug). Don't let that crash the
    # whole coverage run — report it as a module-level compile error and move on.
    run =
      try do
        {wasmf, watf, wrapper_mod} = build(mod, gen)
        vm = Enum.with_index(gen) |> Enum.map(fn {_, i} -> vm_case(wrapper_mod, i) end)
        {:ok, vm, wasm_cases(wasmf, watf, length(gen))}
      rescue
        e -> {:error, e |> Exception.message() |> String.split("\n") |> hd()}
      catch
        kind, e -> {:error, "#{kind}: #{inspect(e) |> String.slice(0, 80)}"}
      end
    ...
    case run do
      ...
      {:error, msg} ->
        IO.puts("     ⚠ COMPILE ERROR (#{length(gen)} fns blocked): #{msg}")
        {mod, 0, length(gen), nogen, length(fns), []}
    end
  end
  ...
  defp build(mod, gen) do
    ...
    File.write!(beam, bin)
    # Feed the broad stdlib closure (minus host-shimmed modules) and let the compiler's DCE prune
    # whatever the wrapper doesn't reach — no hand-curated per-module list, and new roster modules'
    # delegates (Calendar/:filename/…) light up automatically. mod itself is in the closure.
    extra = Tooling.stdlib_closure()
