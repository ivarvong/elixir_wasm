...
- **real dead-code elimination** — the lever for staying under the Workers ~10MB module cap;
...
The prototype uses this exact representation (see `compiler/beam2wasm.exs`):
...
(The prototype already compiled a real interpreter to WasmGC — `compiler/examples/expr.ex` — which is the
