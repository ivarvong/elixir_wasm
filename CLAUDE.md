...
- **Solve it at the root.** Implement the missing piece — a builtin in `lib/codegen_runtime.ex`,
  a host shim in `runtime/imports.mjs`, a compiler lowering in `lib/codegen_emit.ex`, or feeding
...
- "All pure Elixir should run unless there's a *reason* it can't." **`docs/LIMITATIONS.md` is the canonical
...
```
elixir verify.exs        # conformance 219/219 · fuzz 33/33 · gaps 20/20 · genfuzz 12/12 · regexdiff 0 lies ·
                         # scoreboard 487/487 · markdown 3/3 · calc-parser 13/13 · effects — ~15 min
elixir verify.exs fast   # skips the slow suites (scoreboard, markdown) — ~12 min
```
Suites can still be run individually (`cd bench/conformance && elixir run.exs` etc.). When a suite
GROWS, raise its floor in verify.exs as part of the change. Pin the toolchain Node for faithful
diffs: `export NODE=~/.nvm/versions/node/v24.16.0/bin/node` (or rely on `tooling.exs`
...
## Compiler layout
`beam2wasm.exs` is a thin CLI shim over the library in `lib/`:
