# Shared leaf helpers used by BOTH the runtime library (Beam2Wasm.Codegen.Runtime) and the emit path in
...
defmodule Beam2Wasm.Codegen.Common do
  @moduledoc false
  ...
  def term_eq(a, b) do
    ...
    if Process.get(:proc) do
      ...
      eqid = fn t ->
        "(if (result i32) #{both.(t)} (then (i32.eq #{id.(t, a)} #{id.(t, b)})) (else (i32.const 0)))"
      end
  ...
  def dataseg_string(b) do
    ...
    |> Enum.map_join(fn c ->
      if c >= 0x20 and c < 0x7F and c != ?" and c != ?\\,
        do: <<c>>,
        else: "\\" <> Base.encode16(<<c>>, case: :lower)
    end)
  end
  ...
  def type_test_i32(:is_float, vw),
    do: if(Process.get(:float), do: "(ref.test (ref $float) #{vw})", else: "(i32.const 0)")

  def type_test_i32(:is_port, _vw), do: "(i32.const 0)"

  def type_test_i32(:is_integer, vw),
    do:
      if(Process.get(:bignum),
        do:
          "(i32.or (i32.or (ref.test (ref i31) #{vw}) (ref.test (ref $i64) #{vw})) (ref.test (ref $big) #{vw}))",
  ...
  def type_test_i32(:is_boolean, vw),
    do: "(i32.or (ref.eq #{vw} (global.get $atom_true)) (ref.eq #{vw} (global.get $atom_false)))"
end
