...
# beam2wasm.exs (defp -> def). Assembled by Beam2Wasm.run; imports Beam2Wasm.Codegen.Common for the two
# shared leaves it calls (term_eq, bin_literal). Independent of the emit path (AST-confirmed).
defmodule Beam2Wasm.Codegen.Runtime do
  @moduledoc false
  ...
  def bignum_helpers do
    """
      ...
      (func $int_cmp (param $a (ref null eq)) (param $b (ref null eq)) (result i32)  ;; -1/0/1
        (local $ia i64) (local $ib i64)#{if Process.get(:float),
      do: "
...
# :os.system_time/0 — native-unit wall clock from the host (DateTime.utc_now et al.). The host `sys.now`
      ...
      "$file.write_file_2" =>
        if(Process.get(:fs_shim),
          ...
          """,
            ...
            "  (func $file.write_file_2 (param $p (ref null eq)) (param $d (ref null eq)) (result (ref null eq)) (unreachable))"
        ),
      "$file.write_file_3" => """
      ...
      "$sql_host.exec_2" =>
        if(Process.get(:sql_shim),
          ...
          """,
            ...
            "  (func $sql_host.exec_2 (param $q (ref null eq)) (param $p (ref null eq)) (result (ref null eq)) (unreachable))"
        ),
      "$Elixir_46_IO.puts_1" =>
      ...
      "$erlang.error_2" =>
        error_bif(
          "error_2",
          "(param $r (ref null eq)) (param $a1 (ref null eq))",
          "$atom_error",
          "(local.get $r)"
        ),
      "$erlang.error_3" =>
        error_bif(
          "error_3",
          "(param $r (ref null eq)) (param $a1 (ref null eq)) (param $a2 (ref null eq))",
          "$atom_error",
          "(local.get $r)"
        ),
      "$erlang.nif_error_1" =>
        error_bif("nif_error_1", "(param $r (ref null eq))", "$atom_error", "(local.get $r)"),
      ...
      """,
      "$logger.macro_log_3" =>
        "  (func $logger.macro_log_3 (param (ref null eq)) (param (ref null eq)) (param (ref null eq)) (result (ref null eq)) (global.get $atom_ok))",
      "$logger.macro_log_4" =>
        "  (func $logger.macro_log_4 (param (ref null eq)) (param (ref null eq)) (param (ref null eq)) (param (ref null eq)) (result (ref null eq)) (global.get $atom_ok))",
      "$logger.log_3" =>
        "  (func $logger.log_3 (param (ref null eq)) (param (ref null eq)) (param (ref null eq)) (result (ref null eq)) (global.get $atom_ok))",
      "$logger.error_2" =>
        "  (func $logger.error_2 (param (ref null eq)) (param (ref null eq)) (result (ref null eq)) (global.get $atom_ok))",
    ...
    if Process.get(:req_override) do
      ...
      hdrs =
        "(call $map_put #{emap} #{bin_literal("content-type")} (struct.new $cons #{bin_literal("text/html; charset=utf-8")} (ref.null none)))"
  ...
  defp b2i_wat(bignum?) do
    {acc_decl, acc_init, acc_step, ret} =
      if bignum? do
        {"(local $acc (ref null eq))", "(local.set $acc (ref.i31 (i32.const 0)))",
        ...
        {"(local $acc i32)", "(local.set $acc (i32.const 0))",
