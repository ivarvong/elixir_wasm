...
**Whole-program DCE precision — the biggest open compiler problem here.** The DCE is *sound but
imprecise*. It prunes self-contained code byte-identically (mergesort + 12 unrelated modules → same
output), but a single protocol call reaches the consolidated dispatch, which statically links *every*
impl of that protocol — so for real protocol/dispatch-heavy programs it keeps ~everything (`N of N`),
and **small builds come from curating the fed beams (consolidated protocols + a trimmed stdlib), not
from the compiler.** The bridge is diagnosed (`Shop.report → String.Chars.to_string → Date.to_string`);
the fix is RTA/0-CFA devirtualization — gate the consolidated dispatch by which struct types are
actually *constructed*, as a joint type-set ⟷ call-graph fixpoint. A gate is ~built but an unpinned
bug (cross-protocol entanglement keeping a type live) blocks it. GraalVM-native-image-class work, not
a patch. **[compiler]**
