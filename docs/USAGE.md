...
{:beam2wasm, path: "path/to/elixir_wasm", runtime: false}
```
...
It's a small library under **`lib/`** (`Codegen.Common` leaf helpers, `Codegen.Runtime` the WAT
...
```elixir
# mix.exs
{:beam2wasm, path: "../elixir_wasm", runtime: false}
```
...
key-sorted — a documented delta from BEAM's >32-key HAMT order, see `docs/ARCHITECTURE.md` §5 and
