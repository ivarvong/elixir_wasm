...
- `[done→extend]` **Binaries & bitstrings** — a `$binary` term (`(array (mut i8))` wrapped in a struct,
  ...
  threading) are done and bit-exact vs the VM** (`examples/strix.ex`, runs on workerd). Remaining:
...
- `[done]` **Closures** — `make_fun3`/`call_fun`/`call_fun2` lowered to a `$fun` term (table index + captured
  ...
  **real unmodified `Enum`** run over lists (`examples/enum_demo.ex`) — no protocols needed for the
  `is_list` fast paths.
